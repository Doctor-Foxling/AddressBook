...
namespace Foundry {
	...
	//void Foundry::AddressBookImpl::setEntries(const std::vector<Entry>& entries)
	//{
	...
	Shared<Entry> Foundry::AddressBookImpl::addEntry(Shared<Entry>& entry)
	{
	...
	void Foundry::AddressBookImpl::removeEntry(Shared<Entry>& entry)
	{
		const char* fName = entry->getFirstName();
		const char* lName = entry->getLastName();
